...
class SwaModelUniversityEventAttendees extends SwaModelList {
	...
	public function getListQuery() {
		...
		$query->innerJoin(
			$db->quoteName( '#__swa_event_ticket' ) .
			' AS event_ticket ON event.id=event_ticket.event_id'
		);
		$query->innerJoin(
			$db->quoteName( '#__swa_ticket' ) .
			' AS ticket ON event_ticket.id=ticket.event_ticket_id'
		);
